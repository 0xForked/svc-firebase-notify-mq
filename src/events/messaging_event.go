...
import (
	...
	"github.com/aasumitro/svc-firebase-notify/src/helpers"
	"github.com/aasumitro/svc-firebase-notify/src/listeners"
...
func (event MessagingEvent) handleEvent(deliveryMessage []byte)  {
	...
	if fmt.Sprint(msg["type"]) == "SINGLE" {
		listeners.InitSingleMessageListener(
			fmt.Sprint(msg["token"]),
			fmt.Sprint(msg["title"]),
			fmt.Sprint(msg["description"]),
			event.config,
		).SendSingleNotify()
	} else if fmt.Sprint(msg["type"]) == "MULTICAST" {
